...
Computer History is useful because it turns fragmented desktop activity into resumable work context, but the reference product is restricted by platform, account, workspace, and regional availability and cannot be used as a standalone API-key capability. OpenHistory will provide an open, local-first alternative that works as a lightweight macOS menu-bar and Windows system-tray application while exposing the resulting history to any approved AI agent.
...
- Provide deterministic non-AI titles and summaries, with optional pluggable AI summarization that can use an on-device model or a user-configured compatible endpoint.
...
- Store data locally by default, retain raw events for a configurable period that defaults to 48 hours, and keep derived summaries until the user deletes them.
...
- Exclude cloud synchronization, mobile clients, screenshots, OCR, microphone capture, system-audio capture, and autonomous action execution from the first release.
...
- `summary-generation`: Non-AI fallback summaries plus optional, transparent, provider-agnostic AI enrichment.
...
- Adds local SQLite storage, operating-system keychain integration for secrets, an optional summarizer provider interface, and loopback HTTP/MCP endpoints.
