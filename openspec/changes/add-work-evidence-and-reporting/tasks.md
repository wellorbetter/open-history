...
- [x] 2.1 Implement per-repository opt-in storage, suggestion of observed-but-unadded repositories, and removal that stops collection; verify an unadded repository referenced by window activity produces no evidence and that removal deletes prior evidence. (Persistent opt-in storage in `openhistory-storage` (`opted_in_repositories` table, both the encrypted and in-memory backends), wired through `CollectorRuntime::add_repository`/`remove_repository` into the live `ProjectRegistry`, and exposed as Tauri commands. Not done: suggesting a repository observed in window activity but not yet added — there is no such observation-tracking mechanism yet. "Removal deletes prior evidence" for events already persisted is task 3.5/9.3's job, not this one's; this task only stops future resolution against that root, which is verified.)
